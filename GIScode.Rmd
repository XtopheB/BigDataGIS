...
### Matching DHS (Shapefile) and raster files CRS {-} 

We need to know what is the **CRS** (Coordinates Reference System) for the Raster file in order to use the same for the DHS data and to have exact correspondance of the two geographical systems
...
accessData
```
Here the CRS is "*CRS = +proj=longlat +datum=WGS84 +no_defs*" so w know it is using the *WGS84*^[WGS 84 is an Earth-centered, Earth-fixed terrestrial reference system and geodetic datum. WGS 84 is based on a consistent set of constants and model parameters that describe the Earth's siz and shape (see https://en.wikipedia.org/wiki/World_Geodetic_System.]
...
Second we extract the information for each cluster location. 

```{r RasterExtractionAPP, cache = TRUE}
dhs_all2000 <- extract(aPPData,         # Raster file
                       dhsShapeData,    # Shapefile     
...
# There may be several poverty estimates in values in each cluster 
